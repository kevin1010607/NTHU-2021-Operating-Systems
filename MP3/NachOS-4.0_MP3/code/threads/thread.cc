...
{
    if(priority!=149 && totalAgeTick>=1500){
        int oldPriority = priority;
        priority = min(149, priority+10);
        DEBUG(dbgSche, "[C] Tick[" << kernel->stats->totalTicks << "]: Thread [" << ID << "] changes its priority from [" \
...
{
    lastExecTime = (double)(kernel->stats->totalTicks-startTick);
    // runnung -> ready
    if(toReady){
        totalBurstTime += lastExecTime;
        remainingBurstTime = max(0.0, approximateBurstTime-totalBurstTime);
    }
    // runnung -> waiting
    else{
        double oldApproximateBurstTime = approximateBurstTime;
        totalBurstTime += lastExecTime;
        approximateBurstTime = 0.5*totalBurstTime+0.5*approximateBurstTime;
        DEBUG(dbgSche, "[D] Tick[" << kernel->stats->totalTicks << "]: Thread [" << ID << "] update approximate burst time, from: [" \
            << oldApproximateBurstTime << "], add [" << approximateBurstTime-oldApproximateBurstTime << "], to [" << approximateBurstTime << "]");
        totalBurstTime = 0;
